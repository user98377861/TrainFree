...
def slide_process_single(model, tis_det_map_mpp, slide, patch_n_w_l0, patch_n_h_l0, p_s, m_p_s, colors,
    ...
    output_patches_dir = '/hpc2hdd/home/wenjinqi/low_quality_extract/grandqc/output/patches'
    os.makedirs(output_patches_dir, exist_ok=True)  # 确保输出目录存在
    ...
    for he in tqdm(range(patch_n_h_l0), total=patch_n_h_l0):
        ...
        for wi in range(patch_n_w_l0):
            ...
            if np.count_nonzero(td_patch == 0) > 50:  # Check segmentation map
                # Generate patch
                work_patch = slide.read_region((w, h), 0, (p_s, p_s))
                work_patch = work_patch.convert('RGB')
                ...
                pred_str = '_'.join([str(i) for i in np.unique(mask) if i != 1 and i!=7])  # Convert mask to string for hashing
                unique_filename = f"patch_{he}_{wi}_{pred_str}.png" 
                patch_path = os.path.join(output_patches_dir, unique_filename)
